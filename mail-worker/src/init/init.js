...
const init = {
	async init(c) {
		...
		await this.v3DB(c);
		await this.v3_1DB(c);
		await this.ensureCurrentSchema(c);
		await settingService.refresh(c);
		return c.text(t('initSuccess'));
	},

	async ensureCurrentSchema(c) {
		if (!c.env?.db) {
			return;
		}

		await this.addMissingColumns(c, 'email', [
		...
		await this.addMissingColumns(c, 'setting', [
	...
	async addMissingColumns(c, tableName, columns) {
