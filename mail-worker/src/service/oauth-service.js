...
const oauthService = {
	...
	async bindUser(c, params) {
		...
		if (!bindPayload || bindPayload.oauthUserId !== oauthUserId) {
			throw new BizError('OAuth bind token invalid')
		}
	...
	async linuxDoLogin(c, params) {
		...
		if (!userRow) {
			const bindToken = await jwtUtils.generateToken(c, { oauthUserId: oauthRow.oauthUserId }, 600)
